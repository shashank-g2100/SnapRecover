...
export default function MetadataAnalysisCard({
  ...
  // Supports both OLD MongoDB records and NEW normalized records
  const filename =
    metadata?.filename ||
    metadata?.["File Name"] ||
    "—";
  ...
  const formattedFileSize =
    fileSizeKB
      ...
      : fileSizeBytes
      ? `${(Number(fileSizeBytes) / 1024).toFixed(2)} KB`
      : "—";
      ...
      {(metadata?.Make ||
        ...
        <div className="bg-[#060b16] border border-[#1b2a41] rounded-2xl overflow-hidden">
          ...
          <div className="p-5 grid md:grid-cols-2 gap-4">
            ...
            <MetadataItem label="Software" value={metadata?.Software} />
            <MetadataItem label="Date Taken" value={metadata?.DateTime} />
            <MetadataItem label="GPS Latitude" value={metadata?.["GPS Latitude"]} mono />
            <MetadataItem label="GPS Longitude" value={metadata?.["GPS Longitude"]} mono />
            <MetadataItem label="ISO Speed" value={metadata?.ISO} />
            <MetadataItem label="Focal Length" value={metadata?.FocalLength} />
          </div>
        </div>
      )}
